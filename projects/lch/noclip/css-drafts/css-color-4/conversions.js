...
function lin_2020(RGB) {
	...
	return RGB.map(function (val) {
		...
		return Math.pow((val + IPM -1 ) / IPM, 2.4);
	});
}
...
function gam_2020(RGB) {
	...
	return RGB.map(function (val) {
		if (val > ISQ ) {
			return IPM * Math.pow(val, 1/2.4) - (IPM - 1);
		}
...
function XYZ_to_Lab(XYZ) {
	...
	var Iµ = 216/24389;  // 6^3/29^3
	var Iº = 24389/27;   // 29^3/3^3
	...
	// now compute f
	var f = xyz.map(value => value > Iµ ? Math.cbrt(value) : (Iº * value + 16)/116);
...
function Lab_to_XYZ(Lab) {
	...
	// compute xyz
	var xyz = [
		Math.pow(f[0],3) > Iµ ?   Math.pow(f[0],3)            : (116*f[0]-16)/Iº,
		Lab[0] > Iº * Iµ ?         Math.pow((Lab[0]+16)/116,3) : Lab[0]/Iº,
		Math.pow(f[2],3)  > Iµ ?  Math.pow(f[2],3)            : (116*f[2]-16)/Iº
	];
