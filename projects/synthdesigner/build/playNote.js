...
const sus = (linearSustain = 1) => (Math.exp(susCurve) ** linearSustain - 1) / (Math.exp(susCurve) - 1);
...
const releaseNote = (note, release) => {
    // Stop sustain from draining the sound
    note.gainNode.gain.cancelScheduledValues(audioContext.currentTime);
    // I don’t understand why this is needed…
    note.gainNode.gain.setValueAtTime(note.gainNode.gain.value, audioContext.currentTime);
...
export const playNote = ({ frequency = 440, attack: attackMs = 10, waveShape = "sine", forceFullAttack = false, decay: decayMs = 2000, sustain = 0, release: releaseMs = 100, } = {}) => {
