var balancedMatch = function(a, b, str) {
  var bal = 0;
  var m = {};
...
function checkIEEE754 (buf, offset, ext) {
  if (offset + ext > buf.length) throw new RangeError('Index out of range')
...
var debug = src('css-color-function:parse');
...
function parse$1 (string) {
  ...
  function fn () {
    ...
    var colorRef = balancedMatch('(', ')', string);
...
exports.blend = function (color$1, args) {
  var targetAlpha = color$1.alpha();
  ...
  color$1.mix(other, percentage).alpha(targetAlpha);
};
 ...
 * @param {Object} args
 */
exports.contrast = function (color$1, args) {
  if (args.length == 0) args.push({ type: 'argument', value: '100%' });
  var percentage = 1 - parseInt(args[0].value, 10) / 100;
  var max = color$1.luminosity() < .5 ? new color({ h:color$1.hue(), w:100, b:0 }) : new color({ h:color$1.hue(), w:0, b:100 });
  var min = max;
  var minRatio = 4.5;
  if (color$1.contrast(max) > minRatio) {
    var min = binarySearchBWContrast(minRatio, color$1, max);
...
function toRGB (ast) {
  var color$1 = new color(ast.arguments[0].type == "function" ? toRGB(ast.arguments[0]) : ast.arguments[0].value);
  ...
  fns.forEach(function (adjuster) {
    ...
    adjusters[name](color$1, adjuster.arguments);
  });

  return color$1.rgbString();
}
