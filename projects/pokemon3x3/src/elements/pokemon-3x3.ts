import getPokemon, { pokemon } from "../pokemon.js";
import connection from "../connection-details.js";


export interface guessListenerEvent {
  guess: string,
};

interface guessListener {
  options: object,
  callback(event: guessListenerEvent),
};
...
export class Pokemon3x3 extends HTMLElement {
  pokemon: pokemon;
  gridElt: HTMLDivElement;
  nameElt: HTMLDivElement;
  imageElt: HTMLImageElement;
  guessForm: HTMLFormElement;
  pokemonListElt: HTMLDivElement;
  guessInput: HTMLInputElement;
  colorElts: HTMLDivElement[];
  guessListeners: guessListener[];
  connectedCallbackCalled: boolean = false;
  ...
  public set guessing(value: boolean | string) {
  ...
  public set reveal(value: boolean | string) {
    if (value == null || value === false) {
